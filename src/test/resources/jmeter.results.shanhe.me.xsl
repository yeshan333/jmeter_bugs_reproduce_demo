        ...
        <head>
            ...
            <style type="text/css"><![CDATA[
                ...
                * { margin: 0; padding: 0 }
				...
				#left-panel li.navigation1 { font-weight: bold; cursor: default; color: #00aeef; line-height: 18px; background-position: 24px 5px; background-repeat: no-repeat; padding: 0 0 0 35px; background-image: url(data:image/png;base64,) }
            ...
            <div id="left-panel">
                <ol id="result-list">
                    <xsl:for-each select="*">
                        ...
                        <xsl:if test="position() &lt; 3 or @tn != preceding-sibling::*[1]/@tn">
                            <li class="navigation">Thread: <xsl:value-of select="@tn"/></li>
                        </xsl:if>
						<xsl:choose>
						<xsl:when test="httpSample or sample">
						<li class="navigation1">
						Transaction: <xsl:value-of select="@lb" />
                            ...
                            <div>
                                ...
                                <div class="zebra">
                                    <table>
										...
										<td class="data key">Request Data</td><td class="data delimiter">:</td><td class="data"><pre class="data"><xsl:value-of select="queryString"/></pre></td>
										</xsl:when>
										<xsl:otherwise>
										<td class="data key">Request Data</td><td class="data delimiter">:</td><td class="data"><pre class="data"><xsl:value-of select="samplerData"/></pre></td>
										</xsl:otherwise>
										</xsl:choose>
										</tr>
